...
constexpr int MEMORY_OFFSET = (1u << 20) * 16;
constexpr int BENCH_ITER = 100;

constexpr int BLOCK = 128;
constexpr int LDG_UNROLL = 1;

// NOTE: 这里的 uint4 其实是 4 个 int，跟 float4 一样
...
void benchmark(size_t size_in_byte) {
  printf("%luMB (r+w)\n", size_in_byte / (1u << 20));
  double size_gb = (double)size_in_byte / (1u << 30);

  size_t n = size_in_byte / sizeof(uint4);
  size_t grid = n / (BLOCK * LDG_UNROLL);

  static_assert(MEMORY_OFFSET % sizeof(uint4) == 0,
                "Memory offset must be aligned");

  char* ws;
  CHECK_CUDA_ERRORS(cudaMalloc(&ws, size_in_byte + MEMORY_OFFSET * BENCH_ITER));
  CHECK_CUDA_ERRORS(
  ...
  printf("read %fGB/s\n", size_gb * BENCH_ITER / ((double)time_ms / 1000.0));
